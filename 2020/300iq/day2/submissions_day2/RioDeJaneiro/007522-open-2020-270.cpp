...
int main() {
 ...
 for (int i= 0; i <= n; i++) {
       ...
       auto zget = [&] (int i, int l, int r) {
        if (!l) return hsh[i][r];
         ...
         auto lcp = [&] (int i, int j, int k) {
            ...
            while (l < r - 1) {
                int mid = (l + r) / 2;
                if (k + mid < m && zget(i, k, k + mid - 1) == zget(j, k, k + mid - 1)) {
                    l = mid;
                } else {
                    r = mid;
                }
    ...
    for (int i = 0; i + k - 1 < n; i++) {
        for (int j = 0; j + k - 1 < m; j++) {
            if (when[i][j] >= k && bm[i][j] >= k && zm[i][j] >= k) {
                ans++;
            }
        }
    }
