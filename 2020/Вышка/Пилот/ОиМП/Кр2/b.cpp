...
bool TicTacToe::CheckDiagonal1(size_t i, size_t j) const {
    size_t d1 = 0;
    while (i + d1 < N && j + d1 < N && Table[i + d1][j + d1] == Table[i][j]) {
        ++d1;
    }
    size_t d2 = 0;
    while (i >= d2 && j >= d2 && Table[i - d2][j - d2] == Table[i][j]) {
        ++d2;
    }
    return d1 + d2 > K;
}
...
bool TicTacToe::CheckDiagonal2(size_t i, size_t j) const {
    size_t d1 = 0;
    while (i + d1 < N && j >= d1 && Table[i + d1][j - d1] == Table[i][j]) {
        ++d1;
    }
    size_t d2 = 0;
    while (i >= d2 && j + d2 < N && Table[i - d2][j + d2] == Table[i][j]) {
        ++d2;
    }
    return d1 + d2 > K;
}
