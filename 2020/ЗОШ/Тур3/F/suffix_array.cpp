...
void build() {
	...
	//	if (ra[i] == sz - 1) continue;
			//cout << id1 << " " << id2 << " " << id3 << " " << id4 << endl;
			//cout << "ra = " << ra[i] << " i = " << i << " nxt = " << suff[ra[i] + 1] << " len1 = " << len1 << " len2 = " << len2 << endl; 			
	//		chkmin(lcp[ra[i]], min(len1, len2));		
	//}
	/*cout << "lcp = " << endl;
	for (auto i : lcp)
		cout << i << " ";
...
void solve_pref() {
	...
	for (int i = 0; i < sz; i++) {
		...
		if (Help[pos].t < n) {
			...
			int len1 = t[id1].size() - id2;
			int len2 = a[last_pos].size();
			cout << "i = ";
			cout << i << " " << len1 << " " << len2 << endl;
			
			chkmax(ans[id1][id2], min(last_lcp, min(len1, len2)));
			chkmin(last_lcp, lcp[i]);
		}
	}
}
