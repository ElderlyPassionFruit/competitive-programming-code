...
void solve(int pos, int id, bool Flag) {
	int l = 0, r = n;
	while (l < r - 1) {
		int mid = (l + r) / 2;
		bool flag = check(pos - mid) && check(pos + mid - Flag); 
		if (flag) {
			if (id == 0) { 
				flag &= check(pos - mid, pos + mid - Flag, -1);
			}
			else { 
				flag &= check(-1, pos - mid, pos + mid - Flag);
			}
		}
		if (flag)
			l = mid;
	...
	if (id == 0) {
		chkmax(fans, Fans(pos - l, pos + l - Flag, -1));
	}
	else {
		chkmax(fans, Fans(-1, pos - l, pos + l - Flag));
	}
	int j;
	if (id == 0) {
		j = pos + l - Flag;
	...
	while (l < r - 1) {
		...
		bool flag;
		if (id == 0) {
			flag = check(pos - mid) && check(pos + mid - 1 - Flag);
		}
		else {
			flag = check(pos - mid + 1) && check(pos + mid - Flag);
		}
		if (flag) {
			if (id == 0) {
				flag &= check(pos - mid, j, pos + mid - 1 - Flag);
			}
			else {
				flag &= check(pos - mid + 1, j, pos + mid - Flag);
			}
		}
		if (flag)
			l = mid;
	...
	if (id == 0) {
		if (j != pos + l) {
			chkmax(fans, Fans(pos - l, j, pos + l - 1 - Flag));
		}
	}
	else {
		if (j != pos - l) {
			chkmax(fans, Fans(pos - l + 1, j, pos + l - Flag));
		}
	}
}
