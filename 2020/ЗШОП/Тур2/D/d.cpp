...
ll get(int ls, int rs, int lt, int rt) {
 	int pos;
 	if (a[rs] <= lt) return 0;
 	pos = upper_bound(all(a), (ll)lt) - a.begin();
 	ls = max(ls, pos);

    ll ans = 0;

    pos = --upper_bound(all(a), (ll)rt + 1) - a.begin();
    chkmax(pos, ls - 1);
    chkmin(pos, rs);
    //cout << ls << " " << rs << endl;
    //cout << "pos = " << pos << endl;
    if (pos != rs)
    	ans += (ll) (rs - pos) * (rt - lt + 1);
    rs = pos;
    if (rs < 0) return ans;
    ans += pref[rs] - (ls > 0 ? pref[ls - 1] : 0);
    ans -= (ll)(rs - ls + 1) * lt;
    return ans;
}
