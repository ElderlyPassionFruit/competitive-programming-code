...
struct segment_tree_merge{
	...
	segment_tree_merge(const vector<int> & a) {
		n = a.size();
		tree.resize(4 * n);
		build(1, 0, n, a);
	}
	...
	int get(int v, int tl, int tr, int l, int r, int t) {
		...
		if (tl >= l && tr <= r) {
			int pos = upper_bound(all(tree[v]), t) - tree[v].begin();
			return pos;
		}
		int tm = (tl + tr) / 2;
		return get(v * 2, tl, tm, l, r, t) + get(v * 2 + 1, tm, tr, l, r, t);
	}

	int get(int l, int r, int t) {
		chkmax(l, 0);
		return get(1, 0, n, l, r + 1, t);
	}
};
