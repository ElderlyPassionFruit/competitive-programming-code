...
void dfs(int v, int c)
{
	...
	if (c == 1)
	{
		comps.back().first.pbc(v);
	}
	else
	{
		comps.back().second.pbc(v);
	}
	++cnt;
	for (int i : g[v])
	{
...
signed main()
{
	...
	for (int i = 0; i < comps.size(); ++i)
	{
		...
		{
			if (!dp[i][w]) continue;
			if (w + comps[i].first.size() < dp[0].size())
			{
			...
			if (w + comps[i].second.size() < dp[0].size())
			{
	...
	while (nown > 0)
	{
		...
		if (x)
		{
			swap(comps[nown - 1].first, comps[nown - 1].second);
		}
