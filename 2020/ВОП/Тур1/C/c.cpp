...
struct segment_tree{
	...
	int n;
	segment_tree() {}
	segment_tree(const vector<int> & _a, const vector<int> & _have) {
		a = _a;
		have = _have;
		n = a.size();
	...
	int get(int len, int x) {
		...
		for (int i = 0; i < n; i++) {
			if (!tree[i]) continue;
			cerr << "have[i] = " << have[i] << " len = " << len << " x = " << x << " a[i] = " << a[i] << endl;
			if (abs(have[i] + x) % 2 == 0) {
				ans += ((len + 1) / 2) * ((a[i] + 1) / 2) + (len / 2) * (a[i] / 2);
			} else {
				ans += len * a[i] - ((len + 1) / 2) * ((a[i] + 1) / 2) + (len / 2) * (a[i] / 2);
			}
		}
		cerr << "ans = " << ans << endl;
		return ans;
...
void make() {
	...
	for (int i = 1; i < (int)have.size(); i++) {
		d.push_back(have[i] - have[i - 1] + 1);
	}
	tree = segment_tree(d, have);
...
void run() {
	...
	cerr << "a = " << endl;
	for (auto i : a) {
		cerr << "x = " << i.x << " l = " << i.l << " r = " << i.r << " type = " << i.type << endl;
	}
	for (int i = 0; i < (int)a.size(); i++) {
		int pos = i;
		if (i > 0) ans += tree.get(a[i].x - a[i - 1].x, a[i - 1].x);
