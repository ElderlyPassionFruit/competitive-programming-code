...
signed main() {
    ...
    min_cost_max_flow solver(2 * k + 2, 2 * k, 2 * k + 1);
    for (int i = 0; i < k; i++) {
        for (int j = 0; j < k; j++) {
            solver.add_edge(i, j + k, 1, -a[i][j]);
        }
    }
    for (int i = 0; i < k; i++) {
        solver.add_edge(2 * k, i, 1, 0);
    }
    for (int i = 0; i < k; i++) {
        solver.add_edge(i + k, 2 * k + 1, 1, 0);
    }
    cout << abs(solver.get_cost()) << endl;
    for (int i = 0; i < k; i++) {
        for (auto j : solver.g[i]) {
            if (solver.Edge[j].flow != 1) continue;
            cout << fout(solver.Edge[j].to - k);
        }
    }
    cout << endl;
    return 0;
